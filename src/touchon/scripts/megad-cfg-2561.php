	...
	if ( array_key_exists('read-conf', $options) )
	{
		...
		for ( $i = 0; $i < count($pages); $i++ )
		{
			...
			foreach($select as $elem)
			{
				...
				// MCP23017 && PCA9685
				if ( $port_type == 4 && ( $port_dev == 20 || $port_dev == 21 ) )
				{
					//echo "! - $i";
					for ( $j = 0; $j < 16; $j++ )
					$pages[] = "pt=$port_num&ext=$j";
				}
			
			}

			$url = preg_replace("/&$/", "", $url);
			if ( ( !preg_match("/^cf=1&/", $url) && $i < count($pages) - 1 && $i != $reboot_flag ) || preg_match("/^cf=10&/", $url) )
...
if ( array_key_exists('scan', $options) || array_key_exists('s', $options) )
{
	...
	$broadcast_string = chr(0xAA).chr(0).chr(12).chr(0xDA).chr(0xCA);
...
else if ( (array_key_exists('ip', $options) || array_key_exists('f', $options) ) && ( array_key_exists('fw', $options) || array_key_exists('w', $options) ) )
{
	...
	if ( !empty($options['ip']) )
	{
		...
		$bl = @file_get_contents("http://".$options['ip']."/".$options['p']."/?bl=1", 0, $ctx);
		usleep(10000);
	}

	$check_data = '';
	if ( $bl == 1 )
	$check_data = chr(0xDA).chr(0xCA);
	...
	if ( array_key_exists('p', $options) || array_key_exists('f', $options) )
	{
		...
		else
		{
			...
			if ( $megad_check == 1 || array_key_exists('f', $options) || array_key_exists('e', $options) )
			{
				...
				$broadcast_string = chr(0xAA).chr(0).chr(0x00).$check_data;
				if ( array_key_exists('e', $options) )
				{
				...
				if ( ord($pkt[0]) == 0xAA && ord($pkt[1]) == 0x00 )
				{
					...
					//if ( (strlen($firmware) > 28670 && $chip_type == 0 ) || ( strlen($firmware) > 558046 && $chip_type == 2561 )  )
					{
						...
						// Restaring
						$broadcast_string = chr(0xAA).chr(0).chr(0x03).$check_data;
						socket_sendto($sock, $broadcast_string, strlen($broadcast_string), 0, $broadcast_ip, 52000);
						$pkt = stream_socket_recvfrom($socket, 200, 0, $peer);
					...
					$broadcast_string = chr(0xAA).chr(0).chr(0x02).$check_data;
					socket_sendto($sock, $broadcast_string, strlen($broadcast_string), 0, $broadcast_ip, 52000);
					$pkt = stream_socket_recvfrom($socket, 200, 0, $peer);
					//$pkt[0] = chr(0xAA);
					//$pkt[1] = chr(0x00);
					if ( ord($pkt[0]) == 0xAA && ord($pkt[1]) == 0x00 )
					{
						...
						{
							//echo "$msg_id\n";
							$broadcast_string = chr(0xAA).chr($i).chr(0x01).$check_data.$fw_block[$i];
							socket_sendto($sock, $broadcast_string, strlen($broadcast_string), 0, $broadcast_ip, 52000);
							//echo " ->";
							usleep(400);
							//$pkt = stream_socket_recvfrom($socket, 200, 0, $peer);
					...
					{
	                                        echo "Erasing EEPROM... ";
						$broadcast_string = chr(0xAA).chr(0).chr(9).$check_data;
						socket_sendto($sock, $broadcast_string, strlen($broadcast_string), 0, $broadcast_ip, 52000);
						// 2561
						...
						if ( $chip_type == 2561 )
						{
        						$broadcast_string = chr(0xAA).chr(0x01).chr(9).$check_data;
							socket_sendto($sock, $broadcast_string, strlen($broadcast_string), 0, $broadcast_ip, 52000);
							$pkt = stream_socket_recvfrom($socket, 200, 0, $peer);
							//echo ord($pkt[0])." ".ord($pkt[1])." ".ord($pkt[2])."\n";
...
{
	echo "MegaD-2561 management script Ver 2.23\n";
...
if ( (array_key_exists('ip', $options) && array_key_exists('new-ip', $options)) || ( array_key_exists('ee', $options) && array_key_exists('write-conf', $options) ) )
{
	...
	if ( array_key_exists('p', $options) )
	{
		...
		else
		{
			...
			{
				$broadcast_string_old = chr(0xAA).chr(0).chr(4).$broadcast_string;

				socket_sendto($sock, $broadcast_string_old, strlen($broadcast_string_old), 0, $broadcast_ip, 52000);
				usleep(100);
				$pkt = fread($socket, 10);
				if ( ord($pkt[0]) == 0xAA )
				{
				...
				else
				{
					...
					$broadcast_string_new = chr(0xAA).chr(0).chr(4).chr(0xDA).chr(0xCA).$broadcast_string;

					socket_sendto($sock, $broadcast_string_new, strlen($broadcast_string_new), 0, $broadcast_ip, 52000);
					usleep(100);
					$pkt = fread($socket, 10);
					if ( ord($pkt[0]) == 0xAA )
					{
...
if ( array_key_exists('write-conf', $options) && $conf_flag == 1 )
{
	...
	file_get_contents("http://".$options['ip']."/".$options['p']."/?restart=1");
